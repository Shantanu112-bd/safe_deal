...
/// Helper to set up a default test environment with auth mocked
fn setup_env() -> Env {
    let env = Env::default();
    env.mock_all_auths();
    env
...
/// Helper to create a standard test deal
fn create_test_deal(env: &Env, client: &MerchantEscrowContractClient, seller: &Address) -> String {
...
fn test_lock_payment() {
    ...
    let success = client.lock_payment(&deal_id, &buyer, &100_000_000i128);
...
fn test_lock_payment_wrong_amount() {
    ...
    // Try to lock with wrong amount — should return false
    let success = client.lock_payment(&deal_id, &buyer, &90_000_000i128);
    assert_eq!(success, false);

    // Deal should remain in WaitingForPayment
    let deal = client.get_deal(&deal_id);
    assert_eq!(deal.status, DealStatus::WaitingForPayment);
    assert!(deal.buyer.is_none());
}
...
fn test_confirm_delivery_not_locked() {
    ...
    // Try to confirm before payment is locked
    client.confirm_delivery(&deal_id, &buyer);
}
...
fn test_auto_refund_not_locked() {
    ...
    // Deal is still WaitingForPayment — can't auto-refund
...
#[test]
#[should_panic(expected = "Only WaitingForPayment deals can be cancelled")]
fn test_cancel_deal_already_locked() {
    let env = setup_env();
    let contract_id = env.register_contract(None, MerchantEscrowContract);
    let client = MerchantEscrowContractClient::new(&env, &contract_id);
...
fn test_cancel_deal_wrong_seller() {
    ...
    // Impostor tries to cancel — should panic
    client.cancel_deal(&deal_id, &impostor);
}

// ============================================================
// get_deal / get_seller_deals / get_buyer_deals tests
// ============================================================
...
fn test_full_deal_lifecycle() {
    ...
    // 2. Lock payment
    let ok = client.lock_payment(&deal_id, &buyer, &100_000_000i128);
    assert!(ok);
    let deal = client.get_deal(&deal_id);
    assert_eq!(deal.status, DealStatus::Locked);
